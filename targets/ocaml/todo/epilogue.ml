...
let eq = model_eq
let num_todos (m : model) = List.length (snd m)

(* test  # Add adds # *)
(*   num_todos(update(("Breath", []), AddTodo)) *)
(*   > num_todos(("Breath", [])) end; *)
let test1 _ =
  assert_bool "test1 failed"
    (num_todos (update (("Breath", []), AddTodo)) > num_todos ("Breath", []))
...
let test8 _ =
  let model = ("", [ ("1", false) ]) in
  assert_bool "test8 failed"
    (num_todos (update (model, RemoveTodo 0)) < num_todos model)
