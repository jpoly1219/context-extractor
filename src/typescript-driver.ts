...
import {
  ...
  Range,
  SymbolInformation,
} from "../ts-lsp-client-dist/src/main";
...
import {
  ...
  findTypeDeclarationGivenIdentifier,
  getFullLanguageName,
  getQueryForFile,
} from "./tree-sitter";
...
export class TypeScriptDriver implements LanguageDriver {
  ...
  constructor(ide: IDE, sources: string[], projectRoot: string) {
    ...
    this.tsCompilerProgram = this.typeChecker.createTsCompilerProgram(
      sources,
      projectRoot
    );
  ...
  async getHoleContext(
    ...
    if (lspClient) {
      ...
      const formattedHoverResult = (
        holeHoverResult.contents as MarkupContent
      ).value
        .split("\n")
  ...
  async getHoleContextWithCompilerAPI(
    ...
    const sourceFile = this.tsCompilerProgram.getSourceFile(
      injectedSketchFilePath
    )!;
  ...
  async getHoleContextWithTreesitter(
    ...
    const sketchFileContent = fs.readFileSync(sketchFilePath, "utf8");
    const injectedContent = insertAtPosition(
      sketchFileContent,
      cursorPosition,
      "@;"
    );
    ...
    const snip = extractSnippet(injectedContent, cursorPosition, {
  ...
  async extractRelevantTypes1(
    ...
    if (!foundSoFar.has(typeName)) {
      ...
      for (let linePos = startLine; linePos <= endLine; ++linePos) {
        ...
        const numOfCharsInLine = parseInt(
          execSync(`wc -m <<< "${content.split("\n")[linePos]}"`, {
            shell: "/bin/bash",
          }).toString()
        );
        ...
        for (let charPos = 0; charPos < numOfCharsInLine2; ++charPos) {
          try {
            ...
            if (
              ...
              const tdResultStr = JSON.stringify(
                typeDefinitionResult as Location[]
              );
              if (!foundTypeDefinitions.has(tdResultStr)) {
                foundTypeDefinitions.set(
                  tdResultStr,
                  typeDefinitionResult as Location[]
                );
                ...
                const dsMap = documentSymbolResult.reduce((m, obj) => {
                  m.set(
                    (obj as SymbolInformation).location.range.start.line,
                    (obj as SymbolInformation).location
                ...
                const matchingSymbolRange: Range | undefined = dsMap.get(
                  (typeDefinitionResult[0] as Location).range.start.line
                );
                if (matchingSymbolRange) {
                  const snippetInRange = extractSnippet(
                    fs
                      .readFileSync(
                        (typeDefinitionResult[0] as Location).uri.slice(7)
                      )
                      .toString("utf8"),
                    matchingSymbolRange.start,
                    matchingSymbolRange.end
                  );
                  ...
                  const identifier =
                    this.typeChecker.getIdentifierFromDecl(snippetInRange);
  ...
  async extractRelevantTypesHelper(
    ...
    if (lspClient) {
      ...
      if (!foundSoFar.has(typeName)) {
        ...
        const identifiers =
          this.typeChecker.extractIdentifiers(fullHoverResult);
        // DEBUG: REMOVE
          ...
          if (!foundSoFar.has(identifier.name)) {
            try {
              ...
              if (
                ...
                const decl = this.typeChecker.findDeclarationForIdentifier(
                  content,
                  tdLocation.range.start.line,
                  tdLocation.range.start.character,
                  tdLocation.range.end.character
                );
                if (decl) {
          ...
          if (!foundSoFar.has(identifier.name)) {
            try {
              const typeDefinitionResult =
                await this.vscodeImport.VsCode.gotoTypeDefinition({
      ...
      for (const identifier of identifiers) {
        if (!foundSoFar.has(identifier.name)) {
          try {
            ...
            const position = ts.getPositionOfLineAndCharacter(
              sourceFile,
              linePosition + identifier.line - 1,
              identifier.start
            );
            // const typeDefinitionResult = await lspClient.typeDefinition({
            //   textDocument: {
            ...
            const possiblyPrimitiveType =
              this.tsCompilerTypeChecker.getTypeAtLocation(node);
            if (
          ...
          if (!foundSoFar.has(node.text)) {
            try {
              ...
              if (
                ...
                if (!ast) {
                  throw new Error(
                    `failed to get ast for file ${tdLocation.uri}`
                  );
                }
  ...
  async extractRelevantHeaders(
    ...
    const targetTypes = this.generateTargetTypes(
      relevantTypes,
      holeType,
    ...
    for (const source of sources) {
      ...
      // const start = performance.now()
      const varFuncDecls = this.typeChecker.findTopLevelDeclarations(
        this.tsCompilerProgram,
        this.tsCompilerTypeChecker,
        source
      );
  ...
  generateTargetTypesHelper(
    ...
    const typeAnalysisResult = this.typeChecker.analyzeTypeString(
      currType,
      program,
      checker
    );
  ...
  extractRelevantHeadersHelper(
    ...
    if (!foundTypeAnalysisResults.has(source + ":" + typeSpan)) {
      typeAnalysisResult = this.typeChecker.analyzeTypeString(
        typeSpan,
        program,
        checker
      );
      ...
      typeAnalysisResult = foundTypeAnalysisResults.get(
        source + ":" + typeSpan
      )!;
    }
  ...
  normalize2(
    ...
    const analysisResult = this.typeChecker.analyzeTypeString(
      typeSpan,
      program,
      checker
    );
  ...
  async extractRelevantHeadersWithTreesitter(
    ...
    const targetTypes = await this.generateTargetTypesWithTreesitter(
      relevantTypes,
      holeType,
      holeIdentifier
    );
  ...
  async extractRelevantHeadersWithTreesitterHelper(
    ...
    for (const typ of targetTypes) {
      if (
        await this.isTypeEquivalentWithTreesitter(
          node,
          typ,
          relevantTypes,
          foundNormalForms
  ...
  async generateTargetTypesWithTreesitter(
    ...
    await this.generateTargetTypesWithTreesitterHelper(
      relevantTypes,
      holeType,
      targetTypes,
      baseNode
    ...
    switch (node.type) {
      ...
      case "tuple_type": {
        const elements = node.namedChildren.map((c) =>
          this.normalizeWithTreesitter(c!, relevantTypes)
        );
