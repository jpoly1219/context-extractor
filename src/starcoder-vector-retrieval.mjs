...
import { joinFiles, generateStarcoderVectorRetrievalPrompt } from "./testrunner-core.mjs";
...
const rag = process.argv[7];
const errorRoundsMax = process.argv[9];
const sourceFolder = process.argv[11];
const bashTimestamp = process.argv[12];
...
const runID = `${runName}-${source}-vector-${bashTimestamp}`;
const outDirRoot = path.join(projectRoot, "starcoder-vector-retrieval", "out", runID);
fs.mkdirSync(outDirRoot, { recursive: true }, (err) => { if (err) throw err; });
const logDirRoot = path.join(projectRoot, "starcoder-vector-retrieval", "testlog");
const backupDir = path.join(projectRoot, "starcoder-vector-retrieval", "backup", `${runID}`);
fs.mkdirSync(backupDir, { recursive: true }, (err) => { if (err) throw err; });
const combinedFolder = path.join(projectRoot, "targets", "combined");


/* PHASE 2: RAG CONTEXT GENERATION */


execSync(`python3 ${path.join(projectRoot, "chunker2.py")}`, { stdio: 'ignore' })
execSync(`python3 ${path.join(projectRoot, "starcoder_generate_rags.py")}`, { stdio: 'ignore' })
...
const prompt = generateStarcoderVectorRetrievalPrompt(sketchFileContent, ragCtx);
console.log(prompt);
...
fs.writeFileSync(`${path.join(sourceFolder, "vector_llm_completed_sketch.ts")}`, sketchFileContent + llmResult.content + "\n}");
...
for (let i = 0; i < parseInt(errorRoundsMax) + 1; i++) {
  try {
    ...
    execSync(`tsc ${path.join(sourceFolder, "vector_test_suite.ts")}`);
    // console.log(`== ${ res }== `)
    break;
  } catch (err) {
    if (i >= parseInt(errorRoundsMax)) {
      ...
      execSync(`rm -f ${path.join(sourceFolder, "erroneous*")} ${path.join(sourceFolder, "completed_sketch*")} ${path.join(sourceFolder, "vector_llm_completed_sketch*")} ${path.join(sourceFolder, "vector_test_suite*")} ${path.join(sourceFolder, "*.js")}`);
      process.exit(1);
    } else {
